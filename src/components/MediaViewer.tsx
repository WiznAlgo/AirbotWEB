...
export function MediaViewer({ item, index = 0 }: { item: any; index?: number }) {
  ...
  useEffect(() => {
    ...
    mediaToBlob(item).then((blob) => {
      ...
      // Kalau responsnya nggak bawa Content-Type yang jelas (server suka
      // ngirim application/octet-stream buat gambar), paksa jadi image/png
      // supaya Blob URL-nya punya MIME yang benar — ini yang dipakai Chrome
      // buat nebak ekstensi pas user klik-kanan "Simpan gambar".
      const needsFix = item.type === 'image' && (!blob.type || blob.type === 'application/octet-stream');
      const finalBlob = needsFix ? new Blob([blob], { type: 'image/png' }) : blob;
