...
function filenameForMedia(item: any, index: number, blobType?: string) {
  if (item.fileName) return item.fileName;
  if (item.name) return item.name;
  // Urutan prioritas buat nebak mime: field eksplisit di item dulu, baru
  // Content-Type asli dari blob yang udah di-fetch (paling akurat kalau item
  // cuma punya `url` tanpa mimeType), baru fallback berdasarkan item.type,
  // baru terakhir base64 dianggap image/png. Kalau semua kosong, dulu jatuh
  // ke .bin — sekarang default-nya .png karena mayoritas kasus di sini
  // memang gambar hasil create image.
  const mime =
    item.mimeType ||
    item.mimetype ||
    (blobType && blobType !== 'application/octet-stream' ? blobType : '') ||
    (item.base64 ? 'image/png' : '') ||
    (item.type === 'image' ? 'image/png' : '');
...
export async function downloadSingleMedia(item: any, index = 0) {
  ...
  triggerBlobDownload(blob, filenameForMedia(item, index, blob.type));
  return true;
}

// Download semua media dalam 1 output command, selalu dibungkus jadi 1 file
// .zip (biar konsisten walau isinya cuma 1 gambar) — item yang masuk sini
// sudah dipastikan tidak ada duplikat oleh pemanggilnya.
export async function downloadMediaBundle(items: any[], baseName: string) {
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    if (!blob) continue;
    let name = filenameForMedia(valid[i], i, blob.type);
